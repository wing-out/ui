...
const (
	...
	screenshotDir   = "/tmp/wingout_desktop_e2e_screenshots"
)
...
func TestMain(m *testing.M) {
	...
	if binaryPath == "" {
		fmt.Println("WINGOUT_BINARY_PATH not set, skipping desktop E2E tests")
		os.Exit(0)
	}
	...
	fmt.Printf("gRPC server at %s\n", srvAddr)

	// Enable AT-SPI2 in Qt
	os.Setenv("QT_ACCESSIBILITY", "1")
	// Use offscreen platform if no display (CI environments)
	if os.Getenv("DISPLAY") == "" {
		os.Setenv("QT_QPA_PLATFORM", "offscreen")
	}

	atspiClient := NewATSPIClient("wingout")
	...
	if sharedEnv.appCmd != nil && sharedEnv.appCmd.Process != nil {
		_ = sharedEnv.appCmd.Process.Kill()
		_ = sharedEnv.appCmd.Wait()
	}
	srv.Stop()
	srvCancel()
	os.Exit(code)
